...
public class Model {
	...
	public void ModelBuilding() throws FileNotFoundException {
		try {
			...
			// ---constraint 20 ---// if truck k doesn't drive from start node, then it
			// can't appear on other roads
			for (int k = 0; k < numberOfTrucks; k++) {
				IloLinearNumExpr constraint20 = cplex.linearNumExpr();
				for (int i = 0; i < numberOfCities; i++) {
					for (int j = 0; j < numberOfCities; j++) {
						constraint20.addTerm(1, X[i][j][k]);
					}
				}

				for (int m = 0; m < numberOfCities; m++) {
					constraint20.addTerm(-M, X[truckStartNode[k]][m][k]);
				}

				cplex.addLe(constraint20, 0);
//				System.out.println(constraint20.toString());
			}
			
			// ---constraint 21 ---// truck k can't take the same demand when it leave and arrive its start node
			for(int k=0;k<numberOfTrucks;k++) {
				for(int od=0;od<numberOfDemandPair;od++) {
					IloLinearNumExpr constraint21 = cplex.linearNumExpr();
					for(int i=0;i<numberOfCities;i++) {
						constraint21.addTerm(1, x[truckStartNode[k]][i][k][od]);
						constraint21.addTerm(1, x[i][truckStartNode[k]][k][od]);
					}
					cplex.addLe(constraint21, 1);
//					System.out.println(constraint21.toString());
				}
			}
